...
async function readJson(req: Request): Promise<{ key?: string; installId?: string } | null> {
  try {
...
async function fetchLicenseByKey(supabaseUrl: string, serviceRoleKey: string, key: string) {
  return fetch(`${supabaseUrl}/rest/v1/license_keys?key=eq.${encodeURIComponent(key)}&select=id,key_type,used,used_at,activated_device_id,activated_at`, {
    method: 'GET',
    headers: serviceHeaders(serviceRoleKey),
  });
}

async function patchLicenseById(supabaseUrl: string, serviceRoleKey: string, id: string, body: Record<string, unknown>) {
  return fetch(`${supabaseUrl}/rest/v1/license_keys?id=eq.${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: serviceHeaders(serviceRoleKey),
...
Deno.serve(async req => {
  ...
  if (!key) return jsonResponse({ ok: false, error: 'Missing license key.' }, 400);
  if (!installId) return jsonResponse({ ok: false, error: 'Missing install identifier.' }, 400);
  ...
  if (keyType === 'premium') {
    ...
    if (!record.used || !record.used_at || !activationId) {
      const patchResponse = await patchLicenseById(supabaseUrl, serviceRoleKey, String(record.id), {
        used: true,
        used_at: now,
